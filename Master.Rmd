...
Distance_Moyenne <- aggregate(select(Distance.complete,7,11), by=list(Distance.complete$i,Distance.complete$j), FUN = mean, na.rm=TRUE)

#on ajoute un IJ pour la fusion plus tard
Distance_Moyenne$ij <- paste0(Distance_Moyenne$Group.1,Distance_Moyenne$Group.2)
...
M1_RegPanel_EFTemps <- plm(log(v) ~ log(gdp_o)+log(gdp_d)+log(distw)+log(gdpcap_o)+log(gdpcap_d)+contig+comcur+commlang+gatt_o+gatt_d+rta, data=Table_Gravite, index= c("ij","year"), model = c("within"), effect=c("time"), na.action= na.omit)

M1_RegPanel_EFPays <- plm(log(v) ~ log(gdp_o)+log(gdp_d)+log(distw)+log(gdpcap_o)+log(gdpcap_d)+contig+comcur+commlang+gatt_o+gatt_d+rta, data=Table_Gravite, index= c("ij","year"), model = c("within"), effect=c("individual"), na.action= na.omit)
...
stargazer(M1_RegMCO, M1_RegPanel_EFTemps, M1_RegPanel_EFPays ,M1_RegPoiss,column.labels = c("MCO","Panel Effet Fixe Temps","Panel Effet Fixe Pays", "PseudoMaximum de Poisson") ,type = "html")
```
...
#MCO classique
RegMCO96 <- lm(log(v) ~ log(gdp_o)+log(gdp_d)+log(distw)+log(gdpcap_o)+log(gdpcap_d)+contig+comcur+commlang+gatt_o+gatt_d+rta, data= Table_Gravite94_14 , na.action = na.omit)

#Equation de gravite en panel
RegPanelTime96bis <- plm(log(v) ~ log(gdp_o)+log(gdp_d)+log(distw)+log(gdpcap_o)+log(gdpcap_d)+contig+comcur+commlang+gatt_o+gatt_d, data=Table_Gravite94_14, index= c("ij","year"), model = c("within"), effect=c("time"), na.action= na.omit)

RegPanelCountry96 <- plm(log(v) ~ log(gdp_o)+log(gdp_d)+log(distw)+log(gdpcap_o)+log(gdpcap_d)+contig+comcur+commlang+gatt_o+gatt_d, data=Table_Gravite94_14, index= c("ij","year"), model = c("within"), effect=c("individual"), na.action= na.omit)

#Sans utiliser le plm
RegPanelMCO96 <-lm(log(v) ~ log(gdp_o)+log(gdp_d)+log(distw)+log(gdpcap_o)+log(gdpcap_d)+contig+comcur+commlang+gatt_o+gatt_d+factor(year)-1, data= Table_Gravite94_14 , na.action = na.omit)

#En MCO classique on retrouve bien les memes coefs
RegPanel_pool96 <- plm(log(v) ~ log(gdp_o)+log(gdp_d)+log(distw)+log(gdpcap_o)+log(gdpcap_d)+contig+comcur+commlang+gatt_o+gatt_d, data=Table_Gravite94_14, index= c("ij","year"), model = c("pooling"),na.action= na.omit)

#glm poisson
RegPoiss96 <- glm(v ~ log(gdp_o)+log(gdp_d)+log(distw)+log(gdpcap_o)+log(gdpcap_d)+contig+comcur+commlang+gatt_o+gatt_d, data=Table_Gravite94_14, na.action = na.omit, family = quasipoisson(link="log"))
#effet fixe annee
